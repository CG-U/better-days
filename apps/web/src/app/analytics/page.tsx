...
function SectionLabel({
  ...
  return (
    <p
      className={`text-xs font-bold uppercase tracking-widest ${toneClass}`}
    >
      {children}
    </p>
  );
...
export default function AnalyticsPage() {
  ...
  if (analytics.isPending) {
    return (
      <main className="mx-auto w-full max-w-xl p-5 md:p-10">
  ...
  if (analytics.isError) {
    return (
      <main className="mx-auto w-full max-w-xl p-5 md:p-10">
        <p className="text-center text-muted-foreground">
          We could not load your trends. Please refresh to try again.
        </p>
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto flex w-full max-w-xl flex-col gap-6 p-5 md:p-10">
      <header className="space-y-1">
        <h1 className="font-heading text-3xl font-bold">Your Journey</h1>
      ...
      {!data.setupComplete ? (
        ...
        <>
          <section className="rounded-2xl border border-border bg-primary-container/25 p-6">
          ...
          <section className="rounded-2xl border border-border bg-card p-6">
            <div className="flex items-center justify-between">
              ...
              <PiggyBank aria-hidden className="size-5 text-tertiary" />
            </div>
          ...
          <ul className="flex flex-wrap gap-2">
            {data.topTriggers.map((item, index) => (
              <li
                ...
                className={
                  index === 0
                    ? "rounded-full bg-secondary px-4 py-2 text-sm font-semibold text-white"
